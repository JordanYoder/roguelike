...
func get_blocking_entity_at_location(grid_position: Vector2i) -> Entity:
	# Check to see if there is a blocking entity at a specific location
...
func grid_to_index(grid_position: Vector2i) -> int:
	if not is_in_bounds(grid_position):  # Don't return if out of bounds
		return -1
	return grid_position.y * width + grid_position.x
	
	
func register_blocking_entity(entity: Entity) -> void:
	# Tell the pathfinder that the blocking entity is there
...
func unregister_blocking_entity(entity: Entity) -> void:
	# Tell the pathfinder that the blocking entity isn't there
...
func setup_pathfinding() -> void:
	...
	pathfinder.region = Rect2i(0, 0, width, height)  # Get the dimensions of the map
	pathfinder.update()  # Now that we have map dimension, update pathfinder
	
	# Go through each x, y coordinate and denote if tile is not walkable
	for y in height:
		for x in width:
			var grid_position := Vector2i(x, y)
	...
	# Go through each entity and register if the entity is blocking
	for entity in entities:
...
func get_actors() -> Array[Entity]:
	# Gets actors that that are alive and returns them
